    ...
    public class Binder : IBinder
    {
        ...
        public Either<ErrorList, BoundScript> Bind(EnhancedScript script)
        {
            ...
            var eitherMain = script.Functions.FirstOrNone(f => f.Name == "Main").Match(
                _ => Either.Success<ErrorList, bool>(true), () => new ErrorList(new Error(ErrorKind.UndefinedMainFunction, "Main function not defined")));
        ...
        private Either<ErrorList, BoundStatement> Bind(Statement statement)
        {
            ...
            return new ErrorList(new Error(ErrorKind.BindError, $"Cannot bind {statement}"));
        }
        ...
        private Either<ErrorList, BoundExpression> Bind(Expression expression)
        {
            ...
            return new ErrorList(new Error(ErrorKind.BindError, $"Expression '{expression}' could not be bound"));
        }
        ...
        private Either<ErrorList, BoundExpression> Bind(CallExpression call)
        {
            ...
            return context.Functions.FirstOrNone(function => function.Name == call.Name)
                .Match(function => eitherParameters.MapRight(parameters => (BoundExpression)new BoundBuiltInFunctionCallExpression(function, parameters)),
                    () => new ErrorList(new Error(ErrorKind.UndeclaredFunction, $"FunctionDeclaration '{call.Name}' isn't declared")));
        }
